...
void visualization()
{
    ...
    for (int x = 0; x < int(mapWidth/gridWidth); x++) {
        for (int y = 0; y < int(mapHeight/gridHeight); y++) {
            if (l[x][y] == l0) {
                plt::plot({(double)x}, {(double)y}, "g.");
            } else if (l[x][y] >= locc) {
                plt::plot({(double)x}, {(double)y}, "k.");
            } else if (l[x][y] <= lfree) {
                plt::plot({(double)x}, {(double)y}, "r.");
            }
        }
    }
    
    // Save the image and close the plot 
    plt::save("./Images/mapping.png");
    plt::clf();
}
